...
function QueueManagementPage() {
  ...
  useEffect(() => {
    ...
    async function loadMandis() {
      try {
        ...
        setSelectedMandi(operatorMandi)

        if (!operatorMandi) {
          setBookings([])
          setError(
            'No active mandi is assigned to this operator account.',
          )
          return
        }
  ...
  function openQRVerification(bookingCode: string) {
  window.location.href =
    `/mandi-operator/qr-verification?bookingCode=${encodeURIComponent(
      bookingCode,
    )}`
}
  ...
  async function handleCompleteProcessing(id: number) {
    try {
      ...
      if (!selectedMandi) {
        throw new Error('No active mandi is selected.')
      }

      const refreshedBookings =
        await getMandiBookings(selectedMandi.id)
        ...
        <div className="px-4 py-6 mx-auto max-w-7xl sm:px-6 lg:px-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex items-center gap-3 p-4 bg-white border shadow-sm rounded-2xl border-slate-200">
              <div className="flex items-center justify-center h-11 w-11 rounded-xl bg-green-50">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1.5fr_0.8fr]">
                ...
                <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-3 mt-3">
                      <span className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-semibold">
                        {nextItem.vehicle}
                      </span>
                  ...
                  <div className="flex flex-col gap-2 sm:min-w-[190px]">
                    <button
                      ...
                      className="w-full px-5 py-3 text-sm font-semibold text-green-800 transition-all border border-green-200 rounded-xl bg-green-50 hover:bg-green-100 disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      ✓ Verify QR / Token
                    </button>
                    ...
                    <button
                      ...
                      className="w-full px-5 py-3 text-sm font-semibold text-white transition-all bg-blue-600 rounded-xl hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"
                    >
          ...
          <section className="p-6 bg-white border shadow-sm rounded-2xl border-slate-200">
            ...
            <button
              ...
              className="w-full p-4 mt-5 text-left transition-all border border-green-100 rounded-xl bg-green-50 hover:bg-green-100 disabled:cursor-not-allowed disabled:opacity-50"
            >
              ...
              <p className="mt-1 text-xs text-green-700">
                Verify the next confirmed booking before
                processing.
              </p>
            </button>

            <div className="p-4 mt-3 border border-blue-100 rounded-xl bg-blue-50">
                    ...
                    <div className="p-5 md:p-6">
                      <div className="flex flex-col gap-5 md:flex-row md:items-center">
                        <div className="flex items-center gap-3 md:w-36">
                          <div
                            className={`flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-2xl border-2 text-lg font-bold ${
                              item.status ===
                              'completed'
                        ...
                        <div className="flex flex-col gap-2 md:w-40">
                          {item.status === 'next' && (
                            <>
                              <button
                                ...
                                className="w-full px-4 py-3 text-sm font-semibold text-green-800 transition-all border border-green-200 rounded-xl bg-green-50 hover:bg-green-100 disabled:cursor-not-allowed disabled:opacity-50"
                              >
                                ✓ Verify QR / Token
                              </button>
                              ...
                              <button
                                ...
                                className="w-full px-4 py-3 text-sm font-semibold text-white transition-all bg-blue-600 rounded-xl hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"
                              >
                                {actionId === item.id
                                  ? 'Starting...'
                                  : 'Start Processing'}
                          ...
                          {item.status ===
                            ...
                            <button
                              ...
                              className="w-full px-4 py-3 text-sm font-semibold text-white transition-all bg-green-700 rounded-xl hover:bg-green-800 disabled:cursor-not-allowed disabled:opacity-50"
                            >
                              {actionId === item.id
                                ? 'Completing...'
                                : 'Mark Done'}
                          ...
                          {item.status ===
                            'completed' && (
                            <div className="px-4 py-3 text-sm font-semibold text-center border rounded-xl border-slate-200 bg-slate-50 text-slate-500">
                              ✓ Processed
                            </div>
                          )}
                          ...
                          {item.status ===
                            'waiting' && (
                            <div className="px-4 py-3 text-xs font-semibold text-center border rounded-xl border-amber-200 bg-amber-50 text-amber-700">
                              Waiting for turn
                            </div>
                          )}
                        </div>
